...
export function Game({
  ...
  const loadStage = useCallback(() => {
    ...
    const shuffled = [...stageQs].sort(() => Math.random() - 0.5);
  ...
  useEffect(() => {
    if (remainingQuestions.length === 0 && skippedQuestions.length > 0) {
      setRemainingQuestions(skippedQuestions);
      setSkippedQuestions([]);
    }
  }, [remainingQuestions]);
  ...
  const handleOptionClick = (option: string) => {
    if (questionStatus !== "active") return;
  ...
  const handleCorrectAnswer = (option: string) => {
    const team = teams[answeringTeamIndex];

    const newScore = (stageScores[team.id] || 0) + POINTS_PER_QUESTION;
    ...
    setTeams((prev) =>
      prev.map((t) =>
        t.id === team.id ? { ...t, score: t.score + POINTS_PER_QUESTION } : t,
      ),
    );
    ...
    if (newScore >= POINTS_TO_WIN_STAGE) {
      unlockStage(team);
      return;
    }
  ...
  if (!currentQuestion && stageQuestions.length === 0) {
    return (
      <div className="text-center p-8 text-2xl text-white">
        جاري تحميل مرحلة {currentStage?.title}... (تأكد من وجود أسئلة لهذه
        المرحلة)
        <button
          onClick={handleNextStage}
          className="block mx-auto mt-4 px-6 py-2 bg-amber-500 text-slate-900 rounded-lg"
        >
          تخطي المرحلة
        </button>
      </div>
  ...
  return (
    <div className="flex flex-col h-full relative">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {teams.map((team, idx) => {
          const isAnswering =
            questionStatus === "active" && idx === answeringTeamIndex;
          const isOriginalTurn =
            questionStatus === "ready" && idx === activeTeamIndex;
          const stageScore = stageScores[team.id] || 0;
          const progress = Math.min(
            100,
            (stageScore / POINTS_TO_WIN_STAGE) * 100,
          );

          return (
            <div
              key={team.id}
              className={`bg-slate-800/80 backdrop-blur-sm border-2 p-4 rounded-2xl flex flex-col items-center shadow-lg transition-all ${
                isAnswering
            ...
            >
              <div className="text-lg font-bold text-slate-200 mb-1 flex items-center gap-2">
                {team.name}
                {isAnswering && (
                  <span className="text-[10px] bg-amber-500 text-slate-900 px-2 py-0.5 rounded-full animate-pulse">
                    يجيب الآن
                  </span>
                )}
              </div>
              ...
              <div className="text-xs text-slate-400 mb-3">
                {stageScore} / {POINTS_TO_WIN_STAGE} نقطة للمفتاح
              </div>
              ...
              <div className="flex gap-1 h-6">
                {STAGES.map((stage) => (
                  <div
                    key={stage.id}
                    className={`w-6 h-6 flex items-center justify-center rounded-full text-xs ${team.keys.includes(stage.id) ? "bg-amber-500/20 text-amber-400" : "bg-slate-700/50 text-slate-600"}`}
                  >
                    <Key size={12} />
                  </div>
                ))}
        ...
        <AnimatePresence mode="wait">
          {stageUnlocked ? (
            ...
            >
              ...
              <h2 className="text-5xl font-black text-amber-400 mb-4 font-serif drop-shadow-lg">
                تم فتح المرحلة!
              </h2>
              <p className="text-3xl text-slate-200 mb-4">
                الفريق الفائز بالمفتاح:
              </p>
              <p className="text-5xl font-bold text-emerald-400 drop-shadow-lg mb-12">
                {teams[answeringTeamIndex]?.name}
              </p>
              <button
                onClick={handleNextStage}
            ...
            >
              ...
              <h3 className="text-3xl font-bold text-slate-300 mb-6 font-serif">
                السؤال القادم
              </h3>
            ...
            >
              ...
              <div className="bg-slate-950/50 p-6 border-b border-slate-800 flex justify-between items-center">
                <div className="flex items-center gap-4">
                  ...
                  <h2
                    className={`text-3xl font-bold font-serif ${currentStage.color}`}
                  >
                    المرحلة {currentStageIndex + 1}: {currentStage.title}
                  </h2>
                </div>
                <div
                  className={`flex items-center gap-3 font-mono text-4xl font-bold ${
                    timeLeft <= 5 && questionStatus === "active"
              ...
              <div className="p-8 md:p-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {shuffledOptions.map((option, idx) => {
                    let btnClass =
                      "p-6 text-2xl font-medium rounded-2xl border-2 text-center transition-all duration-300 ";
                    const isWrong = wrongOptions.includes(option);

                    if (questionStatus === "active") {
                      if (isWrong) {
                        btnClass +=
                          "border-red-500/50 bg-red-900/20 text-red-400/50 cursor-not-allowed";
                      } else {
                        btnClass +=
                          "border-slate-600 bg-slate-800 text-slate-200 cursor-pointer hover:bg-slate-700 hover:border-slate-500";
                      }
                    } else {
                      if (option === currentQuestion?.correct) {
                        btnClass +=
                          "border-emerald-500 bg-emerald-900/40 text-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.3)] scale-105";
                      } else if (isWrong) {
                        btnClass +=
                          "border-red-900/50 bg-red-950/30 text-red-500/50";
                      } else {
                        btnClass +=
                          "border-slate-700 bg-slate-900/50 text-slate-500 opacity-50";
                      }
                    }
        ...
        <div className="container mx-auto flex flex-wrap justify-center gap-4">
          {questionStatus === "active" && !stageUnlocked && (
            <button
              ...
              className="px-6 py-3 bg-slate-600 hover:bg-slate-500 text-white rounded-xl font-bold flex items-center gap-2 transition-colors"
            >
          ...
          {questionStatus === "revealed" && !stageUnlocked && (
            <button
              ...
              className="px-8 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-bold text-lg flex items-center gap-2 transition-colors shadow-[0_0_15px_rgba(79,70,229,0.4)]"
            >
